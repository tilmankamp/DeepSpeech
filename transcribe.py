...
def transcribe(path_pairs, create_model, try_loading):
    ...
    with tf.Session(config=Config.session_config) as session:
        ...
        def run_transcription(p_index, p_data_set, p_audio_path, p_tlog_path):
            ...
            transcripts = list(map(lambda t: {'start': int(t[0]), 'end': int(t[1]), 'transcript': t[2]}, transcripts))
...
def main(_):
    ...
    if src_path.endswith('.catalog'):
        ...
        catalog_entries = list(map(lambda e: (resolve(catalog_dir, e['audio']), resolve(catalog_dir, e['tlog'])),
                                   catalog_entries))
